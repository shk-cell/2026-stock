...
function updateTradeButtonStatus() {
  ...
  if (isFresh) {
    ...
    $("expireMsg").textContent = "⚠️ 시세 만료 (갱신 필요)";
...
async function fetchQuote() {
  ...
  try {
    ...
    } else { alert("종목 오류"); $("qOutBox").style.display = "none"; }
  } catch { alert("조회 실패"); }
...
async function updateLeaderboard(totalAsset) {
  ...
  // 랭킹 업데이트 시 닉네임도 같이 저장하여 랭킹창에서 바로 보이게 함
  const nickname = $("userNickname").textContent;
  await setDoc(doc(db, "users", user.email), { totalAsset, nickname, lastActive: serverTimestamp() }, { merge: true });
  await fetchRanking();
}
...
async function refreshEverything() {
  ...
  try {
    ...
    const cash = userSnap.exists() ? userData.cash : START_CASH;
      ...
      listHtml += `
        <div class="portfolio-item">
          <div style="display:flex; flex-direction:column; gap:2px;">
            ...
            <div style="font-size:12px; color:var(--muted);">매수 ${money(avgPrice)} | <span style="color:${profitRate>=0?'var(--up)':'var(--down)'}; font-weight:bold;">${profitRate}%</span></div>
          </div>
    ...
    $("portfolioList").innerHTML = listHtml || '<div style="text-align:center; color:var(--muted); padding:20px;">보유 주식이 없습니다.</div>';
...
// 매수/매도 로직은 이전과 동일 (quickSell, buyBtn.onclick 등)
...
onAuthStateChanged(auth, (user) => {
  if (user) { $("authView").classList.add("hidden"); $("dashView").classList.remove("hidden"); refreshEverything(); }
  else { $("authView").classList.remove("hidden"); $("dashView").classList.add("hidden"); }
});

document.addEventListener("DOMContentLoaded", () => {
  $("loginBtn").onclick = () => signInWithEmailAndPassword(auth, $("email").value, $("pw").value).catch(() => $("authMsg").textContent = "아이디 또는 비밀번호가 틀렸습니다.");
